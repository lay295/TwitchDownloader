...
public partial class MainWindow : Window {
    ...
    private void btnVodDownload_Click(object sender, RoutedEventArgs e) {
        this.Main.Content = MainWindow.pageVodDownload;
    }

    private void btnClipDownload_Click(object sender, RoutedEventArgs e) {
        this.Main.Content = MainWindow.pageClipDownload;
    }

    private void btnChatDownload_Click(object sender, RoutedEventArgs e) {
        this.Main.Content = MainWindow.pageChatDownload;
    }

    private void btnChatUpdate_Click(object sender, RoutedEventArgs e) {
        this.Main.Content = MainWindow.pageChatUpdate;
    }

    private void btnChatRender_Click(object sender, RoutedEventArgs e) {
        this.Main.Content = MainWindow.pageChatRender;
    }

    private void btnQueue_Click(object sender, RoutedEventArgs e) { this.Main.Content = MainWindow.pageQueue; }

    private void Window_OnSourceInitialized(object sender, EventArgs e) { App.RequestAppThemeChange(); }
    ...
    private async void Window_Loaded(object sender, RoutedEventArgs e) {
        ...
        Settings.Default.TemplateVod = Regex.Replace(
            Settings.Default.TemplateVod,
            "{crop_(?=(?:start|end)(?:_|}))",
            "{trim_"
        );
        Settings.Default.TemplateClip = Regex.Replace(
            Settings.Default.TemplateClip,
            "{crop_(?=(?:start|end)(?:_|}))",
            "{trim_"
        );
        Settings.Default.TemplateChat = Regex.Replace(
        ...
        this.FlashTaskbarIconIfNotForeground(TimeSpan.FromSeconds(3));

        var currentVersion = Version.Parse("1.54.7");
#if DEBUG
        this.Title = $"Twitch Downloader v{currentVersion} - DEBUG";
#else
        ...
        if (!File.Exists("ffmpeg.exe")) {
            ...
            try {
                ...
                var messageBoxResult = MessageBox.Show(
                    this,
                    string.Format(
                        Strings.UnableToDownloadFfmpegFull,
                        "https://ffmpeg.org/download.html",
                        Path.Combine(Environment.CurrentDirectory, "ffmpeg.exe")
                    ),
                    Strings.UnableToDownloadFfmpeg,
        ...
        public void Report(ProgressInfo value) {
            ...
            if (percent > this._lastPercent) {
                ...
                window.Title = string.Concat(
                    ...
                    "- ",
                    string.Format(Strings.StatusDownloaderFFmpeg, percent.ToString())
                );
            }
        }
    }
