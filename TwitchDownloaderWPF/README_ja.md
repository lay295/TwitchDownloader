...
ほとんどのページでは、ウィンドウの左側に情報エリアがあり、サムネイル（ある場合）、配信者、作成日、VOD/クリップのタイトルが表示され、ウィンドウの中央にはジョブの設定があり、ウィンドウの右側にはイベントのログを表示するエリアがあります。

### VOD のダウンロード
Twitch から配信の VOD やハイライトをダウンロードする。

![Figure 1.1](Images/vodExample.png)
<br><sup>*図 1.1*</sup>

はじめに、VOD またはハイライトの有効なリンク、または ID を入力し、`取得`ボタンを押します。  
すると、ダウンロードオプションのロックが解除され、ジョブをカスタマイズできるようになります。  
VOD またはハイライトがプライベート、またはサブスクライバー限定の場合は、ビデオにアクセスできるアカウントの Oauth トークンが必要です。
...
**OAuth**: サブスクライバー限定およびプライベート VOD やハイライトのダウンロードをするための認証トークンです。有料コンテンツやプライベートコンテンツの不正ダウンロードを防ぐため、このアプリケーションとTwitchが要求しています。Oauth トークンの取得方法は以下の動画（英語）を確認してください。[https://youtu.be/1MBsUoFGuls](https://www.youtube.com/watch?v=1MBsUoFGuls).  
...
はじめに、VOD、ハイライト、クリップの有効なリンク、または ID を入力し、`取得`ボタンを押します。    
すると、ダウンロードオプションのロックが解除され、ジョブをカスタマイズできるようになります。  
VOD やハイライトがサブスクライバー限定、またはプライベートの場合、チャットをダウンロードすることはできません。これは TwitchDownloader の制限ではなく、TwitchAPI の制限です。

**ダウンロードフォーマット**: ダウンロードしたチャットが保存されるファイルの形式。
- `JSON` はチャットの更新やレンダリングに使用できるチャットデータを出力します。
- `Text` は VOD を見ながらチャットを読むのに最適な生のテキストを出力します。
...
**VODのダウンロード**: 同時に行える VOD/ハイライト のダウンロードジョブ数。
...
**URLリスト**: 全て同じ設定で処理される VOD、ハイライト、クリップのURLリスト。図 6.2 および 図 6.5 参照。

**VODs を検索**: 同じ設定で処理される配信者からの全VODを検索するウィンドウ。図 6.3 および 図 6.5 参照。
...
**キャッシュフォルダ**: 一時的な作業ファイルが保存されるフォルダ。これには VOD ダウンロード、スタンプ、バッジ、ビッツが含まれます。
- **クリア**: TwitchDownloader 関連のキャッシュファイルをすべて削除します。アプリケーションが正しく動作していない場合をのぞき、お勧めしません。
- **Browse**: 一時キャッシュを保存するフォルダを選択します。既存のキャッシュファイルは転送されません。
...
- サムネイルが見つかりません
   - VODの有効期限が切れているか、現在ライブ中です。
- ビデオ/クリップ情報が取得できません
   - リンク先のVOD/Clipが無効であるか、削除されているか、プライベート/サブ専用で有効なOAuthが提供されていません。
- 入力を解析できませんでした
   - 1つ以上のレンダー入力が無効です。
