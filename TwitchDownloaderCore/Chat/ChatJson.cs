...
public static class ChatJson {
    ...
    public static async Task<ChatRoot> DeserializeAsync(
        string filePath,
        bool getComments = true,
        bool onlyFirstAndLastComments = false,
        bool getEmbeds = true,
        ...
        if (getEmbeds) {
            if (jsonDocument.RootElement.TryGetProperty("embeddedData", out var embeddedDataElement)) {
                if (returnChatRoot.FileInfo.Version > new ChatRootVersion(1, 2, 2))
                    returnChatRoot.embeddedData
                        = embeddedDataElement.Deserialize<EmbeddedData>(ChatJson._jsonSerializerOptions);
                else {
                    var legacyEmbeddedData
                        = embeddedDataElement.Deserialize<LegacyEmbeddedData>(ChatJson._jsonSerializerOptions);
                    returnChatRoot.embeddedData = new() {
                        ...
                        twitchBadges = legacyEmbeddedData
                                ?.twitchBadges.Select(
                                    item => new EmbedChatBadge {
                                        name = item.name,
                                        versions = item
                                            .versions.Select(
                                                x => new KeyValuePair<string, ChatBadgeData>(
                                                    x.Key,
                                                    new() { Bytes = x.Value }
                                                )
                                            )
                ...
                returnChatRoot.embeddedData = emotesElement.Deserialize<EmbeddedData>(ChatJson._jsonSerializerOptions);
        }
    ...
    private static async Task<JsonDocument> GetJsonDocumentAsync(
        Stream stream,
        string filePath,
        JsonDocumentOptions deserializationOptions,
        CancellationToken cancellationToken = default
    ) {
        if (!stream.CanSeek)
...
#pragma warning disable CS0618 // Type or member is obsolete
    private static async Task UpgradeChatJson(ChatRoot chatRoot) {
        const int
            MAX_STREAM_LENGTH = 172_800; // 48 hours in seconds. https://help.twitch.tv/s/article/broadcast-guidelines
        ...
        if (chatRoot.video.created_at == default)
            chatRoot.video.created_at
                = firstComment?.created_at - TimeSpan.FromSeconds(firstComment?.content_offset_seconds ?? 0) ?? default;
        ...
        if (chatRoot.streamer is null) {
            ...
            var assumedName = chatRoot.video.user_name
                ?? broadcaster.Value?.commenter.display_name ?? await TwitchHelper.GetStreamerName(assumedId);

            chatRoot.streamer = new() { id = assumedId, name = assumedName };
        }
        ...
        switch (compression) {
            case ChatCompression.None:
                await JsonSerializer.SerializeAsync(
                    fileStream,
                    chatRoot,
                    ChatJson._jsonSerializerOptions,
                    cancellationToken
                );
                break;
            ...
            case ChatCompression.Gzip: {
                ...
                await JsonSerializer.SerializeAsync(gs, chatRoot, ChatJson._jsonSerializerOptions, cancellationToken);
                break;
            }
