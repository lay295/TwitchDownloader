...
public static class StreamExtensions {
    ...
    public static async Task ProgressCopyToAsync(
        this Stream source,
        Stream destination,
        long? sourceLength,
        ...
        var rentedBuffer = ArrayPool<byte>.Shared.Rent(StreamExtensions.STREAM_DEFAULT_BUFFER_LENGTH);

        long totalBytesRead = 0;
        try {
            int bytesRead;
            while ((bytesRead = await source
                    .ReadAsync(rentedBuffer, 0, rentedBuffer.Length, cancellationToken)
                    .ConfigureAwait(false))
                != 0) {
                await destination.WriteAsync(rentedBuffer, 0, bytesRead, cancellationToken).ConfigureAwait(false);

                totalBytesRead += bytesRead;
                progress.Report(new(sourceLength.Value, totalBytesRead));
            }
        } finally {
