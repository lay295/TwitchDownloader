...
public static class M3U8Extensions {
    ...
    private static readonly Regex UserQualityStringRegex = new(
        @"(?:^|\s)(?:(?<Width>\d{3,4})x)?(?<Height>\d{3,4})p?(?<Framerate>\d{1,3})?(?:$|\s)",
    ...
    public static M3U8.Stream GetStreamOfQuality(this M3U8 m3u8, string qualityString) {
        ...
        var qualityStringMatch = M3U8Extensions.UserQualityStringRegex.Match(qualityString);
        ...
        var filteredStreams = m3u8
            .Streams
            .WhereOnlyIf(x => x.StreamInfo.Resolution.Width == int.Parse(desiredWidth.ValueSpan), desiredWidth.Success)
            .WhereOnlyIf(
                x => x.StreamInfo.Resolution.Height == int.Parse(desiredHeight.ValueSpan),
                desiredHeight.Success
            )
            .WhereOnlyIf(
                x => Math.Abs(x.StreamInfo.Framerate - int.Parse(desiredFramerate.ValueSpan)) <= 2,
                desiredFramerate.Success
            )
            .ToArray();
    ...
    /// <returns>
    ///     A <see cref="string" /> representing the <paramref name="stream" />'s
    ///     <see cref="M3U8.Stream.ExtStreamInfo.Resolution" />
    ///     and <see cref="M3U8.Stream.ExtStreamInfo.Framerate" /> in the format of "{resolution}p{framerate}" or
    ///     <see langword="null" />
    /// </returns>
    ...
    public static M3U8.Stream BestQualityStream(this M3U8 m3u8) {
        ...
        return source
            ?? m3u8.Streams.MaxBy(
                x => x.StreamInfo.Resolution.Width * x.StreamInfo.Resolution.Height * x.StreamInfo.Framerate
            );
    }
