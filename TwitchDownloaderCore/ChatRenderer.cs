...
namespace TwitchDownloaderCore
{
    public sealed partial class ChatRenderer : IDisposable
    {
        ...
        private static readonly System.Buffers.SearchValues<char> DigitChars = System.Buffers.SearchValues.Create("0123456789");
        ...
        private List<ChatBadge> badgeList = [];
        private List<TwitchEmote> emoteList = [];
        private List<TwitchEmote> emoteThirdList = [];
        private List<CheerEmote> cheermotesList = [];
        private Dictionary<string, SKBitmap> emojiCache = [];
        private Dictionary<string, SKBitmap> avatarCache = [];
        ...
        private void RenderVideoSection(int startTick, int endTick, FfmpegProcess ffmpegProcess, FfmpegProcess maskProcess = null, CancellationToken cancellationToken = new())
        {
            ...
            BinaryWriter ffmpegStream = new(ffmpegProcess.StandardInput.BaseStream);
        ...
        private (SKBitmap frame, bool isCopyFrame) DrawAnimatedEmotes(SKBitmap updateFrame, List<CommentSection> comments, int currentTick)
        {
            ...
            using (SKCanvas frameCanvas = new(newFrame))
            {
        ...
        {
            SKBitmap newFrame = new(renderOptions.ChatWidth, renderOptions.ChatHeight);
        ...
        {
            SKBitmap finalBitmap = new(renderOptions.ChatWidth, sectionImages.Sum(x => x.info.Height));
        ...
        private void DrawText(string drawText, SKPaint textFont, bool padding, List<(SKImageInfo info, SKBitmap bitmap)> sectionImages, ref Point drawPos, Point defaultPos, bool highlightWords, bool noWrap = false)
        {
            ...
            using (SKCanvas sectionImageCanvas = new(sectionImages.Last().bitmap))
            {
                if (highlightWords)
                {
                    using var paint = new SKPaint { Color = Purple};
        ...
        {
            List<(SKBitmap, ChatBadgeType)> returnList = [];
        ...
        private void AddImageSection(List<(SKImageInfo info, SKBitmap bitmap)> sectionImages, ref Point drawPos, Point defaultPos)
        {
            ...
            drawPos.Y = defaultPos.Y;
            SKBitmap newBitmap = new(renderOptions.ChatWidth, renderOptions.SectionHeight);
        ...
        private async Task<Dictionary<string, SKBitmap>> GetScaledEmojis(CancellationToken cancellationToken)
        {
            ...
            foreach (var emojiKey in emojiKeys)
            {
                ...
                SKImageInfo imageInfo = new((int)(newHeight / (double)oldEmojiInfo.Height * oldEmojiInfo.Width), newHeight);
        ...
        private SKPaint GetFallbackFont(int input)
        {
            ...
            SKPaint newPaint = new() { Typeface = fontManager.MatchCharacter(input), LcdRenderText = true, TextSize = (float)renderOptions.FontSize, IsAntialias = true, SubpixelText = true, IsAutohinted = true, HintingLevel = SKPaintHinting.Full, FilterQuality = SKFilterQuality.High };
        ...
        {
            List<string> finalWords = new(words.Length);
            Stack<string> rtlStack = new();
