...
namespace TwitchDownloaderCore
{
    public class ChatRenderer
    {
        ...
        public static SKBitmap DrawMessage(SKBitmap sectionImage, List<SKBitmap> imageList, ChatRenderOptions renderOptions, List<GifEmote> currentGifEmotes, SKPaint messageFont, Dictionary<string, SKBitmap> emojiCache, Dictionary<string, SKBitmap> chatEmotes, List<ThirdPartyEmote> thirdPartyEmotes, List<CheerEmote> cheerEmotes, Comment comment, Size canvasSize, ref Point drawPos, ref int default_x, List<SKBitmap> emoteList, List<SKRect> emotePositionList)
        {
        ...
        public static SKBitmap DrawText(SKBitmap sectionImage, string message, SKPaint messageFont, List<SKBitmap> imageList, ChatRenderOptions renderOptions, List<GifEmote> currentGifEmotes, Size canvasSize, ref Point drawPos, bool padding, int default_x)
        {
            ...
            using (SKCanvas sectionImageCanvas = new SKCanvas(sectionImage))
            {
                if (renderOptions.Outline)
                {
                    ...
                    SKPaint outlinePaint = new SKPaint() { Style = SKPaintStyle.Stroke, StrokeWidth = (float)(4 * renderOptions.EmoteScale), StrokeJoin = SKStrokeJoin.Round, Color = SKColors.Black, IsAntialias = true, LcdRenderText = true, SubpixelText = true, HintingLevel = SKPaintHinting.Full, FilterQuality = SKFilterQuality.High };
                    sectionImageCanvas.DrawPath(outlinePath, outlinePaint);
                }
