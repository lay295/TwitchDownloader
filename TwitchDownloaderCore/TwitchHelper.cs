...
public static class TwitchHelper {
    ...
    private static readonly string[] BttvZeroWidth =
    ...
    public static async Task<GqlVideoResponse> GetVideoInfo(long videoId) {
        var request = new HttpRequestMessage {
            RequestUri = new("https://gql.twitch.tv/gql"),
            Method = HttpMethod.Post,
            Content = new StringContent(
                "{\"query\":\"query{video(id:\\\""
                + videoId
                + "\\\"){title,thumbnailURLs(height:180,width:320),createdAt,lengthSeconds,owner{id,displayName},viewCount,game{id,displayName,boxArtURL},description}}\",\"variables\":{}}",
                Encoding.UTF8,
                "application/json"
            )
        };
        request.Headers.Add("Client-ID", "kimne78kx3ncx6brgo4mv6wki5h1ko");
        using var response = await TwitchHelper.httpClient.SendAsync(request, HttpCompletionOption.ResponseHeadersRead);
    ...
    public static async Task<string> GetVideoPlaylist(long videoId, string token, string sig) {
        ...
        try {
            request = new() {
                RequestUri = new(
                    $"https://usher.ttvnw.net/vod/{videoId}.m3u8?sig={sig}&token={token}&allow_source=true&allow_audio_only=true&platform=web&player_backend=mediaplayer&playlist_include_framerate=true&supported_codecs=av1,h264"
                ),
                Method = HttpMethod.Get
            };
            response = await TwitchHelper.httpClient.SendAsync(request);
        } catch (Exception ex) {
            if (IsAuthException(ex)) {
                request = new() {
                    RequestUri = new(
                        $"https://twitch-downloader-proxy.twitcharchives.workers.dev/{videoId}.m3u8?sig={sig}&token={token}&allow_source=true&allow_audio_only=true&platform=web&player_backend=mediaplayer&playlist_include_framerate=true&supported_codecs=av1,h264"
                    ),
                    Method = HttpMethod.Get
                };
                response = await TwitchHelper.httpClient.SendAsync(request);
        ...
        if (response.StatusCode == HttpStatusCode.Forbidden) {
            ...
            if (forbiddenResponse.Contains("vod_manifest_restricted")
                    || forbiddenResponse.Contains("unauthorized_entitlements"))
    ...
    public static async Task<GqlClipResponse> GetClipInfo(object clipId) {
        var request = new HttpRequestMessage {
            ...
            Method = HttpMethod.Post,
            Content = new StringContent(
                "{\"query\":\"query{clip(slug:\\\""
                + clipId
                + "\\\"){title,thumbnailURL,createdAt,durationSeconds,broadcaster{id,displayName},videoOffsetSeconds,video{id},viewCount,game{id,displayName,boxArtURL}}}\",\"variables\":{}}",
                Encoding.UTF8,
                "application/json"
            )
        };
    ...
    public static async Task<GqlClipTokenResponse> GetClipLinks(string clipId) {
        var request = new HttpRequestMessage {
            ...
            Method = HttpMethod.Post,
            Content = new StringContent(
                "{\"operationName\":\"VideoAccessToken_Clip\",\"variables\":{\"slug\":\""
                + clipId
    ...
    public static async Task<GqlVideoSearchResponse> GetGqlVideos(
        string channelName,
        string cursor = "",
        int limit = 50
    ) {
        var request = new HttpRequestMessage {
            ...
            Content = new StringContent(
                ...
                + ""
                + (cursor == "" ? "" : ",after:\\\"" + cursor + "\\\"")
                + ") { edges { node { title, id, lengthSeconds, previewThumbnailURL(height: 180, width: 320), createdAt, viewCount, game { id, displayName } }, cursor }, pageInfo { hasNextPage, hasPreviousPage }, totalCount }}}\",\"variables\":{}}",
                Encoding.UTF8,
                "application/json"
            )
        };
    ...
    public static async Task<GqlClipSearchResponse> GetGqlClips(
        string channelName,
        string period = "LAST_WEEK",
        string cursor = "",
        int limit = 50
    ) {
        var request = new HttpRequestMessage {
            ...
            Content = new StringContent(
                ...
                + ", criteria: { period: "
                + period
                + " }) {  edges { cursor, node { id, slug, title, createdAt, durationSeconds, thumbnailURL, viewCount, game { id, displayName } } }, pageInfo { hasNextPage, hasPreviousPage } }}}\",\"variables\":{}}",
                Encoding.UTF8,
                "application/json"
            )
        };
    ...
    public static async Task<EmoteResponse> GetThirdPartyEmotesMetadata(
        int streamerId,
        bool getBttv,
        bool getFfz,
        bool getStv,
        bool allowUnlistedEmotes,
        ITaskLogger logger,
    ...
    private static async Task<List<EmoteResponseItem>> GetBttvEmotesMetadata(
        int streamerId,
        CancellationToken cancellationToken
    ) {
        var globalEmoteRequest = new HttpRequestMessage(
            HttpMethod.Get,
            new Uri("https://api.betterttv.net/3/cached/emotes/global", UriKind.Absolute)
        );
        using var globalEmoteResponse = await TwitchHelper.httpClient.SendAsync(
            globalEmoteRequest,
        ...
        try {
            var channelEmoteRequest = new HttpRequestMessage(
                HttpMethod.Get,
                new Uri($"https://api.betterttv.net/3/cached/users/twitch/{streamerId}", UriKind.Absolute)
            );
            using var channelEmoteResponse = await TwitchHelper.httpClient.SendAsync(
                channelEmoteRequest,
                HttpCompletionOption.ResponseHeadersRead,
                cancellationToken
            );
            ...
            var bttvChannel
                = await channelEmoteResponse.Content.ReadFromJsonAsync<BTTVChannelEmoteResponse>(
                    cancellationToken: cancellationToken
                );
        ...
        return (
            ...
            let id = emote.id
            let name = emote.code
            let mime = emote.imageType
            let url = $"https://cdn.betterttv.net/emote/{id}/[scale]x"
            select new EmoteResponseItem {
                Id = id,
                Code = name,
                ImageType = mime,
                ImageUrl = url,
                IsZeroWidth = TwitchHelper.BttvZeroWidth.Contains(name)
            }).ToList();
    }

    private static async Task<List<EmoteResponseItem>> GetFfzEmotesMetadata(
        int streamerId,
        CancellationToken cancellationToken
    ) {
        var globalEmoteRequest = new HttpRequestMessage(
            HttpMethod.Get,
            new Uri("https://api.betterttv.net/3/cached/frankerfacez/emotes/global", UriKind.Absolute)
        );
        using var globalEmoteResponse = await TwitchHelper.httpClient.SendAsync(
            globalEmoteRequest,
            HttpCompletionOption.ResponseHeadersRead,
        ...
        try {
            var channelEmoteRequest = new HttpRequestMessage(
                HttpMethod.Get,
                new Uri($"https://api.betterttv.net/3/cached/frankerfacez/users/twitch/{streamerId}", UriKind.Absolute)
            );
            using var channelEmoteResponse = await TwitchHelper.httpClient.SendAsync(
                channelEmoteRequest,
                HttpCompletionOption.ResponseHeadersRead,
            ...
            var channelEmotes
                = await channelEmoteResponse.Content.ReadFromJsonAsync<List<FFZEmote>>(
                    cancellationToken: cancellationToken
                );
        ...
        return (
            from emote in FFZ
            let id = emote.id.ToString()
            let name = emote.code
            let mime = emote.imageType
            let url = emote.animated
    ...
    private static async Task<List<EmoteResponseItem>> GetStvEmotesMetadata(
        int streamerId,
        bool allowUnlistedEmotes,
        ITaskLogger logger,
        CancellationToken cancellationToken
        ...
        var globalEmoteRequest = new HttpRequestMessage(
            HttpMethod.Get,
            new Uri("https://7tv.io/v3/emote-sets/global", UriKind.Absolute)
        );
        using var globalEmoteResponse = await TwitchHelper.httpClient.SendAsync(
            globalEmoteRequest,
            HttpCompletionOption.ResponseHeadersRead,
        ...
        var globalEmoteObject
            = await globalEmoteResponse.Content.ReadFromJsonAsync<STVGlobalEmoteResponse>(
                cancellationToken: cancellationToken
            );
        ...
        try {
            var streamerEmoteRequest = new HttpRequestMessage(
                HttpMethod.Get,
                new Uri($"https://7tv.io/v3/users/twitch/{streamerId}", UriKind.Absolute)
            );
            using var streamerEmoteResponse = await TwitchHelper.httpClient.SendAsync(
                streamerEmoteRequest,
                HttpCompletionOption.ResponseHeadersRead,
                cancellationToken
            );
            streamerEmoteResponse.EnsureSuccessStatusCode();

            var streamerEmoteObject
                = await streamerEmoteResponse.Content.ReadFromJsonAsync<STVChannelEmoteResponse>(
                    cancellationToken: cancellationToken
                );
        ...
        foreach (var stvEmote in stvEmotes) {
            var emoteData = stvEmote.data;
            var emoteHost = emoteData.host;
            var emoteFiles = emoteHost.files;
            ...
            var emoteFormat = "";
            if (emoteFiles.Any(fileItem => fileItem.format.Equals("webp", StringComparison.OrdinalIgnoreCase)))
                emoteFormat = "webp";

            if (
                emoteFormat is "") // SkiaSharp does not yet properly support avif, only allow webp - see issue lay295#426
            {
                logger.LogVerbose(
                    $"{stvEmote.name} is not available in webp, skipping. Available formats: {string.Join(", ", emoteFiles.Select(x => x.format))}"
                );
                continue;
            }

            var emoteFlags = emoteData.flags;
            if ((emoteFlags & StvEmoteFlags.ContentTwitchDisallowed) != 0
                || (emoteFlags & StvEmoteFlags.Private) != 0) {
            ...
            var emoteResponse = new EmoteResponseItem
                { Id = stvEmote.id, Code = stvEmote.name, ImageType = emoteFormat, ImageUrl = emoteUrl };
    ...
    public static async Task<List<TwitchEmote>> GetThirdPartyEmotes(
        ...
        bool ffz = true,
        bool stv = true,
        bool allowUnlistedEmotes = true,
        bool offline = false,
        CancellationToken cancellationToken = default
    ) {
        var returnList = new List<TwitchEmote>();
        ...
        if (embeddedData?.thirdParty != null)
            foreach (var emoteData in embeddedData.thirdParty) {
                cancellationToken.ThrowIfCancellationRequested();

                try {
                    var newEmote = new TwitchEmote(
                        emoteData.data,
                        EmoteProvider.ThirdParty,
                        emoteData.imageScale,
                        emoteData.id,
                        emoteData.name
                    );
                    ...
                    logger.LogVerbose(
                        $"An exception occurred while loading embedded emote '{emoteData.name}': {e.Message}."
                    );
                }
            }
        ...
        var bttvFolder = new DirectoryInfo(Path.Combine(cacheFolder, "bttv"));
        var ffzFolder = new DirectoryInfo(Path.Combine(cacheFolder, "ffz"));
        var stvFolder = new DirectoryInfo(Path.Combine(cacheFolder, "stv"));

        var emoteDataResponse = await GetThirdPartyEmotesMetadata(
            streamerId,
            bttv,
            ffz,
            stv,
        ...
        if (bttv)
            try {
                await FetchEmoteImages(
                    comments,
                    emoteDataResponse.BTTV,
                    returnList,
                    alreadyAdded,
                    bttvFolder,
                    logger,
                ...
                logger.LogError(
                    $"BetterTTV returned HTTP {e.StatusCode}. BTTV emotes may not be present for this session."
                );
            }
        ...
        static async Task FetchEmoteImages(
            IReadOnlyCollection<Comment> comments,
            IEnumerable<EmoteResponseItem> emoteResponse,
            ICollection<TwitchEmote> returnList,
            ICollection<string> alreadyAdded,
            ...
            foreach (var emote in emoteResponseQuery)
                try {
                    var imageData = await GetImage(
                        cacheFolder,
                        emote.ImageUrl.Replace("[scale]", "2"),
                        emote.Id,
                        2,
                        emote.ImageType,
                        logger,
    ...
    public static async Task<List<TwitchEmote>> GetEmotes(
        ...
        var returnList = new List<TwitchEmote>();
        var alreadyAdded = new List<string>();
        var failedEmotes = new List<string>();

        var emoteFolder = new DirectoryInfo(Path.Combine(cacheFolder, "emotes"));
        ...
        if (embeddedData?.firstParty != null)
            foreach (var emoteData in embeddedData.firstParty) {
                cancellationToken.ThrowIfCancellationRequested();

                try {
                    var newEmote = new TwitchEmote(
                        emoteData.data,
                        EmoteProvider.FirstParty,
                        emoteData.imageScale,
                        emoteData.id,
                        emoteData.name
                    );
        ...
        foreach (var comment in comments.Where(c => c.message.fragments != null)) {
            ...
            foreach (var id in comment
                ...
                try {
                    ...
                    var newEmote = new TwitchEmote(bytes, EmoteProvider.FirstParty, 2, id, id);
                    alreadyAdded.Add(id);
                    returnList.Add(newEmote);
                } catch (HttpRequestException ex) when (ex.StatusCode == HttpStatusCode.NotFound) {
                    failedEmotes.Add(id);
    ...
    public static async Task<List<EmbedChatBadge>> GetChatBadgesData(
        List<Comment> comments,
        int streamerId,
        ...
        var globalBadgeRequest = new HttpRequestMessage {
            ...
            Method = HttpMethod.Post,
            Content = new StringContent(
                "{\"query\":\"query{badges{imageURL(size:DOUBLE),description,title,setID,version}}\",\"variables\":{}}",
                Encoding.UTF8,
                "application/json"
            )
        };
        globalBadgeRequest.Headers.Add("Client-ID", "kimne78kx3ncx6brgo4mv6wki5h1ko");
        using var globalBadgeResponse = await TwitchHelper.httpClient.SendAsync(
            globalBadgeRequest,
            HttpCompletionOption.ResponseHeadersRead,
            cancellationToken
        );
        ...
        var globalBadges
            = (await globalBadgeResponse.Content.ReadFromJsonAsync<GqlGlobalBadgeResponse>(
                cancellationToken: cancellationToken
            ))
            .data.badges.GroupBy(x => x.name)
        ...
        var subBadgeRequest = new HttpRequestMessage {
            ...
            Content = new StringContent(
                "{\"query\":\"query{user(id: "
                + streamerId
                + "){broadcastBadges{imageURL(size:DOUBLE),description,title,setID,version}}}\",\"variables\":{}}",
                Encoding.UTF8,
                "application/json"
            )
        };
        subBadgeRequest.Headers.Add("Client-ID", "kimne78kx3ncx6brgo4mv6wki5h1ko");
        using var subBadgeResponse = await TwitchHelper.httpClient.SendAsync(
            subBadgeRequest,
            HttpCompletionOption.ResponseHeadersRead,
            cancellationToken
        );
        ...
        var subBadges
            = (await subBadgeResponse.Content.ReadFromJsonAsync<GqlSubBadgeResponse>(
                cancellationToken: cancellationToken
            ))
            .data.user.badges.GroupBy(x => x.name)
            .ToDictionary(x => x.Key, x => x.ToList());

        var badges = new List<EmbedChatBadge>();

        var nameList = comments
        ...
        foreach (var name in nameList) {
            ...
            badges.Add(new() { name = name, versions = versions });
        }

        return badges;
    }

    public static async Task<List<ChatBadge>> GetChatBadges(
        List<Comment> comments,
        int streamerId,
        string cacheFolder,
        ITaskLogger logger,
        ...
        var returnList = new List<ChatBadge>();
        var alreadyAdded = new List<string>();

        // Load our embedded data from file
        if (embeddedData?.twitchBadges != null)
            foreach (var data in embeddedData.twitchBadges) {
                cancellationToken.ThrowIfCancellationRequested();

                try {
                    var newBadge = new ChatBadge(data.name, data.versions);
                    returnList.Add(newBadge);
                    alreadyAdded.Add(data.name);
                } catch (Exception e) {
                    logger.LogVerbose(
                        $"An exception occurred while loading embedded badge '{data.name}': {e.Message}."
                    );
                }
            }
        ...
        var badgesData = await GetChatBadgesData(comments, streamerId, cancellationToken);

        var badgeFolder = new DirectoryInfo(Path.Combine(cacheFolder, "badges"));
        ...
        foreach (var badge in badgesData)
            try {
                ...
                foreach (var (version, data) in badge.versions) {
                    var id = data.Url.Split('/')[^2];
                    var bytes = await GetImage(badgeFolder, data.Url, id, 2, "png", logger, cancellationToken);
                    versions.Add(
                        version,
                        new() {
                            Title = data.Title,
                ...
                returnList.Add(new(badge.name, versions));
            } catch (HttpRequestException ex) when (ex.StatusCode == HttpStatusCode.NotFound) { }
    ...
    public static async Task<Dictionary<string, SKBitmap>> GetEmojis(
        ...
        var emojiExtensions = new Regex(
            @"\.(?:png|PNG)$",
            RegexOptions.RightToLeft
        ); // Extensions are case sensitive on Linux and Mac

        if (!Directory.Exists(emojiFolder))
            CreateDirectory(emojiFolder);

        var emojiFiles = Directory
            .GetFiles(emojiFolder)
            .Where(i => emojiExtensions.IsMatch(i))
            .ToArray();
        ...
        if (emojiFiles.Length < emojiVendor.EmojiCount()) {
            ...
            try {
                ...
                var emojis = archive.Entries
                    .Where(
                        x => !string.IsNullOrWhiteSpace(x.Name) && Path.GetDirectoryName(x.FullName) == emojiAssetsPath
                    );
                ...
                foreach (var emoji in emojis) {
                    ...
                    try {
                        ...
                        /* Being written by a parallel process? */
                    }
                }

                emojiFiles = Directory
                    .GetFiles(emojiFolder)
                    .Where(i => emojiExtensions.IsMatch(i))
                    .ToArray();
            } finally {
    ...
    public static async Task<List<CheerEmote>> GetBits(
        List<Comment> comments,
        string cacheFolder,
        string channelId,
        ITaskLogger logger,
        ...
        var returnList = new List<CheerEmote>();
        var alreadyAdded = new List<string>();

        // Load our embedded data from file
        if (embeddedData?.twitchBits != null)
            foreach (var data in embeddedData.twitchBits) {
                cancellationToken.ThrowIfCancellationRequested();

                try {
                    var tierList = new List<KeyValuePair<int, TwitchEmote>>();
                    var newEmote = new CheerEmote { prefix = data.prefix, tierList = tierList };
                    tierList.AddRange(
                        from tier in data.tierList
                        let tierEmote
                            = new TwitchEmote(
                                tier.Value.data,
                                EmoteProvider.FirstParty,
                                tier.Value.imageScale,
                                tier.Value.id,
                                tier.Value.name
                            )
                        select new KeyValuePair<int, TwitchEmote>(tier.Key, tierEmote)
                    );
                    ...
                    logger.LogVerbose(
                        $"An exception occurred while loading embedded cheermote '{data.prefix}': {e.Message}."
                    );
                }
            }
        ...
        var request = new HttpRequestMessage {
            ...
            Method = HttpMethod.Post,
            Content = new StringContent(
                "{\"query\":\"query{cheerConfig{groups{nodes{id, prefix, tiers{bits}}, templateURL}},user(id:\\\""
                + channelId
                + "\\\"){cheer{cheerGroups{nodes{id,prefix,tiers{bits}},templateURL}}}}\",\"variables\":{}}",
                Encoding.UTF8,
                "application/json"
            )
        };
        request.Headers.Add("Client-ID", "kimne78kx3ncx6brgo4mv6wki5h1ko");
        using var cheerResponseMessage = await TwitchHelper.httpClient.SendAsync(
            request,
            HttpCompletionOption.ResponseHeadersRead,
            cancellationToken
        );
        ...
        var cheerResponse
            = await cheerResponseMessage.Content.ReadFromJsonAsync<GqlCheerResponse>(
                cancellationToken: cancellationToken
            );

        var bitFolder = new DirectoryInfo(Path.Combine(cacheFolder, "bits"));
        if (!bitFolder.Exists)
        ...
        {
            var groupList = cheerResponse.data.cheerConfig.groups.ToList();

            if (cheerResponse.data.user?.cheer?.cheerGroups != null)
                groupList.AddRange(cheerResponse.data.user.cheer.cheerGroups);

            foreach (var cheerGroup in groupList) {
                var templateUrl = cheerGroup.templateURL;

                var cheerNodesQuery =
                    from node in cheerGroup.nodes
                ...
                foreach (var node in cheerNodesQuery) {
                    var prefix = node.prefix;
                    try {
                        var tierList = new List<KeyValuePair<int, TwitchEmote>>();
                        var newEmote = new CheerEmote { prefix = prefix, tierList = tierList };
                        foreach (var tier in node.tiers) {
                            var minBits = tier.bits;
                            var url = templateUrl
                            ...
                            tierList.Add(new(minBits, emote));
                        }
    ...
    public static FileInfo ClaimFile(
        string path,
        Func<FileInfo, FileInfo> fileAlreadyExistsCallback,
        ITaskLogger logger
    ) {
    ...
    public static void CleanUpClaimedFile(
        [AllowNull] FileInfo fileInfo,
        [AllowNull] FileStream fileStream,
        ITaskLogger logger
    ) {
        if (fileInfo is null)
            return;
    ...
    public static void SetDirectoryPermissions(string path) {
        ...
        folderInfo.FileAccessPermissions = FileAccessPermissions.AllPermissions;
        folderInfo.Refresh();
    }
    ...
    /// </summary>
    public static async Task CleanupAbandonedVideoCaches(
        string cacheFolder,
        Func<DirectoryInfo[], DirectoryInfo[]> itemsToDeleteCallback,
        ITaskLogger logger
    ) {
        ...
        var oldVideoCaches = (
                from directory in allCacheDirectories
    ...
    public static int TimestampToSeconds(string input) {
        ...
        var returnSpan = TimeSpan.Parse(input);
    ...
    public static async Task<string> GetStreamerName(int id) {
        try {
            var info = await GetUserInfo(new List<string> { id.ToString() });
    ...
    public static async Task<GqlUserInfoResponse> GetUserInfo(IEnumerable<string> idList) {
        var request = new HttpRequestMessage {
            ...
            Method = HttpMethod.Post,
            Content = new StringContent(
                "{\"query\":\"query{users(ids:["
                + string.Join(",", idList.Select(x => "\\\"" + x + "\\\"").ToArray())
                + "]){id,login,createdAt,updatedAt,description,profileImageURL(width:300)}}\",\"variables\":{}}",
                Encoding.UTF8,
                "application/json"
            )
        };
    ...
    public static async Task<byte[]> GetImage(
        ...
        imageBytes = await TwitchHelper.httpClient.GetByteArrayAsync(imageUrl, cancellationToken);
    ...
    public static async Task<GqlVideoChapterResponse> GetVideoChapters(long videoId) {
        var request = new HttpRequestMessage {
            ...
            Method = HttpMethod.Post,
            Content = new StringContent(
                "{\"extensions\":{\"persistedQuery\":{\"sha256Hash\":\"8d2793384aac3773beab5e59bd5d6f585aedb923d292800119e03d40cd0f9b41\",\"version\":1}},\"operationName\":\"VideoPlayer_ChapterSelectButtonVideo\",\"variables\":{\"videoID\":\""
                + videoId
                + "\"}}",
                Encoding.UTF8,
                "application/json"
        ...
        chapterResponse.data.video.moments ??= new() { edges = new() };

        // When downloading VODs of currently-airing streams, the last chapter lacks a duration
        if (chapterResponse.data.video.moments.edges.LastOrDefault() is { } lastEdge
            && lastEdge.node.durationMilliseconds is 0)
            lastEdge.node.durationMilliseconds
                = lastEdge.node.video.lengthSeconds * 1000 - lastEdge.node.positionMilliseconds;
    ...
    public static async Task<GqlVideoChapterResponse> GetOrGenerateVideoChapters(long videoId, VideoInfo videoInfo) {
        ...
        if (chapterResponse.data.video.moments.edges.Count == 0)
            chapterResponse.data.video.moments.edges.Add(
                GenerateVideoMomentEdge(
                    0,
                    videoInfo.lengthSeconds,
                    videoInfo.game?.id,
                    videoInfo.game?.displayName,
                    videoInfo.game?.displayName,
                    videoInfo.game?.boxArtURL
                )
            );
    ...
    public static VideoMomentEdge GenerateClipChapter(Clip clipInfo) => GenerateVideoMomentEdge(
        0,
        clipInfo.durationSeconds,
        clipInfo.game?.id,
        clipInfo.game?.displayName,
        clipInfo.game?.displayName,
        clipInfo.game?.boxArtURL
    );

    private static VideoMomentEdge GenerateVideoMomentEdge(
        int startSeconds,
        int lengthSeconds,
        string gameId = null,
