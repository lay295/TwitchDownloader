...
public sealed class ClipDownloader {
    ...
    public ClipDownloader(ClipDownloadOptions clipDownloadOptions, ITaskProgress progress) {
        this.downloadOptions = clipDownloadOptions;
    ...
    private async Task DownloadAsyncImpl(
        ...
        this._progress.SetStatus("Fetching Clip Info");

        var downloadUrl = await this.GetDownloadUrl();
        var clipInfo = await TwitchHelper.GetClipInfo(this.downloadOptions.Id);

        cancellationToken.ThrowIfCancellationRequested();

        this._progress.SetTemplateStatus("Downloading Clip {0}%", 0);

        if (!this.downloadOptions.EncodeMetadata) {
            await DownloadFileTaskAsync(
                downloadUrl,
                outputFs,
                this.downloadOptions.ThrottleKib,
                new Progress<StreamCopyProgress>(DownloadProgressHandler),
                cancellationToken
            );
            return;
        }

        if (!Directory.Exists(this.downloadOptions.TempFolder))
            TwitchHelper.CreateDirectory(this.downloadOptions.TempFolder);

        var tempFile = Path.Combine(
            this.downloadOptions.TempFolder,
            $"{this.downloadOptions.Id}_{DateTimeOffset.UtcNow.Ticks}.mp4"
        );
        try {
            ...
            this._progress.SetTemplateStatus("Encoding Clip Metadata {0}%", 0);

            var clipChapter = TwitchHelper.GenerateClipChapter(clipInfo.data.clip);
            await this.EncodeClipWithMetadata(
                tempFile,
            ...
            if (!outputFileInfo.Exists || outputFileInfo.Length == 0) {
                ...
                this._progress.LogError(
                    "Unable to serialize metadata. The download has been completed without custom metadata."
                );
            }
    ...
    private async Task<string> GetDownloadUrl() {
        var listLinks = await TwitchHelper.GetClipLinks(this.downloadOptions.Id);
        ...
        foreach (var quality in clip.videoQualities)
            if (quality.quality
                + "p"
                + (Math.Round(quality.frameRate) == 30 ? "" : Math.Round(quality.frameRate).ToString("F0"))
                == this.downloadOptions.Quality)
        ...
        return downloadUrl
            ...
            + "&token="
            + HttpUtility.UrlEncode(clip.playbackAccessToken.value);
    }

    private static async Task DownloadFileTaskAsync(
        string url,
        FileStream fs,
        int throttleKib,
        IProgress<StreamCopyProgress> progress,
        CancellationToken cancellationToken
    ) {
        var request = new HttpRequestMessage(HttpMethod.Get, url);
        using var response = await ClipDownloader
        ...
        if (throttleKib == -1) {
            ...
            await contentStream
                .ProgressCopyToAsync(fs, contentLength, progress, cancellationToken)
                .ConfigureAwait(false);
        } else {
            await using var contentStream = await response.Content.ReadAsStreamAsync(cancellationToken);
    ...
    private async Task EncodeClipWithMetadata(
        string inputFile,
        string destinationFile,
        Clip clipMetadata,
        VideoMomentEdge clipChapter,
        CancellationToken cancellationToken
    ) {
        ...
        try {
            await FfmpegMetadata.SerializeAsync(
                metadataFile,
                clipMetadata.broadcaster?.displayName,
                this.downloadOptions.Id,
                clipMetadata.title,
                clipMetadata.createdAt,
                clipMetadata.viewCount,
                videoMomentEdges: new[] { clipChapter },
                cancellationToken: cancellationToken
            );
            ...
            process = new() {
                StartInfo = {
                    ...
                    Arguments
                        = $"-i \"{inputFile}\" -i \"{metadataFile}\" -map_metadata 1 -y -c copy \"{destinationFile}\"",
