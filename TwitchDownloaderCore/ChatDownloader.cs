...
public sealed class ChatDownloader {
    ...
    private static readonly HttpClient HttpClient = new() {
        BaseAddress = new("https://gql.twitch.tv/gql"),
    ...
    public ChatDownloader(ChatDownloadOptions chatDownloadOptions, ITaskProgress progress) {
        this.downloadOptions = chatDownloadOptions;
    ...
    private static async Task<List<Comment>> DownloadSection(
        double videoStart,
        double videoEnd,
        string videoId,
        ...
        while (latestMessage < videoEnd) {
            ...
            try {
                ...
                using (var httpResponse = await ChatDownloader
                    .HttpClient.SendAsync(request, HttpCompletionOption.ResponseHeadersRead, cancellationToken)
                    .ConfigureAwait(false)) {
                    httpResponse.EnsureSuccessStatusCode();
                    commentResponse
                        = await httpResponse.Content.ReadFromJsonAsync<GqlCommentResponse[]>(
                            options: null,
                            cancellationToken
                        );
                }
    ...
    private static List<Comment> ConvertComments(CommentVideo video, ChatFormat format) {
        ...
        foreach (var comment in video.comments.edges) {
            ...
            const int
                AVERAGE_WORD_LENGTH
                    = 5; // The average english word is ~4.7 chars. Round up to partially account for spaces
    ...
    public async Task DownloadAsync(CancellationToken cancellationToken) {
        if (string.IsNullOrWhiteSpace(this.downloadOptions.Id))
    ...
    private async Task DownloadAsyncImpl(
        ...
        var downloadType = this.downloadOptions.Id.All(char.IsDigit) ? DownloadType.Video : DownloadType.Clip;

        var (chatRoot, connectionCount) = await this.InitChatRoot(downloadType);
        ...
        for (var i = 0; i < connectionCount; i++) {
            ...
            downloadTasks.Add(
                DownloadSection(
                    ...
                    videoId,
                    taskProgress,
                    this.downloadOptions.DownloadFormat,
                    cancellationToken
                )
            );
        }
        ...
        if (this.downloadOptions.EmbedData && this.downloadOptions.DownloadFormat is ChatFormat.Json or ChatFormat.Html)
            await this.EmbedImages(chatRoot, cancellationToken);

        cancellationToken.ThrowIfCancellationRequested();

        if (this.downloadOptions.DownloadFormat is ChatFormat.Json)
            await this.BackfillUserInfo(chatRoot);

        this._progress.SetStatus("Writing output file");
        switch (this.downloadOptions.DownloadFormat) {
            case ChatFormat.Json:
                await ChatJson.SerializeAsync(outputFs, chatRoot, this.downloadOptions.Compression, cancellationToken);
                break;
            ...
            case ChatFormat.Html:
                await ChatHtml.SerializeAsync(
                    ...
                    chatRoot,
                    this._progress,
                    this.downloadOptions.EmbedData,
                    cancellationToken
                );
                break;

            case ChatFormat.Text:
                await ChatText.SerializeAsync(outputFs, chatRoot, this.downloadOptions.TimeFormat);
                break;

            default:
                throw new NotSupportedException(
                    $"{this.downloadOptions.DownloadFormat} is not a supported output format."
                );
        }
    }
    ...
    private async Task<(ChatRoot chatRoot, int connectionCount)> InitChatRoot(DownloadType downloadType) {
        ...
        var videoId = this.downloadOptions.Id;
        int connectionCount;
        ...
        if (downloadType == DownloadType.Video) {
            ...
            chatRoot.video.description = videoInfoResponse
                .data.video.description?.Replace("  \n", "\n")
                .Replace("\n\n", "\n")
                .TrimEnd();
            chatRoot.video.title = videoInfoResponse.data.video.title;
            chatRoot.video.created_at = videoInfoResponse.data.video.createdAt;
            chatRoot.video.start = this.downloadOptions.TrimBeginning ? this.downloadOptions.TrimBeginningTime : 0.0;
            chatRoot.video.end = this.downloadOptions.TrimEnding
            ...
            connectionCount = this.downloadOptions.DownloadThreads;
            ...
            this.downloadOptions.TrimBeginning = true;
            this.downloadOptions.TrimBeginningTime = (int)clipInfoResponse.data.clip.videoOffsetSeconds;
            this.downloadOptions.TrimEnding = true;
            this.downloadOptions.TrimEndingTime
                = this.downloadOptions.TrimBeginningTime + clipInfoResponse.data.clip.durationSeconds;
            ...
            chatRoot.video.end = (int)clipInfoResponse.data.clip.videoOffsetSeconds
    ...
    private async Task EmbedImages(ChatRoot chatRoot, CancellationToken cancellationToken) {
        this._progress.SetTemplateStatus("Downloading Embed Images {0}%", 0);
        chatRoot.embeddedData = new();
        ...
        foreach (var newEmote in thirdPartyEmotes.Select(
                emote => new EmbedEmoteData {
                    id = emote.Id,
        ...
        foreach (var newEmote in firstPartyEmotes.Select(
                emote => new EmbedEmoteData {
                    id = emote.Id,
                    imageScale = emote.ImageScale,
                    data = emote.ImageData,
        ...
        foreach (var newBadge in twitchBadges.Select(
                badge => new EmbedChatBadge {
                    name = badge.Name,
    ...
    private async Task BackfillUserInfo(ChatRoot chatRoot) {
        ...
        this._progress.SetTemplateStatus("Backfilling Commenter Info {0}%", 0);
        ...
        for (var i = 0; i < userIds.Length; i += BATCH_SIZE)
            try {
                ...
                this._progress.ReportProgress((int)percent);
            } catch (Exception e) {
                this._progress.LogVerbose(
                    $"An error occurred while backfilling commenters {i}-{i + BATCH_SIZE}: {e.Message}"
                );
                failedInfo = true;
            }
        ...
        if (failedInfo)
            this._progress.LogInfo("Failed to backfill some commenter info");
