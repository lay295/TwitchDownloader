...
public sealed class HighlightIcons : IDisposable {
    ...
    private const string BIT_BADGE_TIER_NOTIFICATION_ICON_SVG
        = "M 14.242705,42.37453 36,11.292679 57.757295,42.37453 36,61.023641 Z M 22.566425,41.323963 36,22.13092 49.433577,41.317747 46.79162,43.580506 36,39.266345 25.205273,43.586723 22.566425,41.320854 Z";

    private const string WATCH_STREAK_ICON_SVG
    ...
    private static readonly Regex SubMessageRegex = new(
        @"^((?:\w+ )?subscribed (?:with Prime|at Tier \d)\. They've subscribed for \d{1,3} months(?:, currently on a \d{1,3} month streak)?! )(.+)$",
        RegexOptions.Compiled
    );

    private static readonly Regex GiftAnonymousRegex = new(
        @"^An anonymous user (?:gifted a|is gifting \d{1,4}) Tier \d",
        RegexOptions.Compiled
    );

    private static readonly Regex WatchStreakRegex = new(
    ...
    public HighlightIcons(ChatRenderOptions renderOptions, SKColor iconPurple, SKPaint outlinePaint) {
        this._cacheDir = new(Path.Combine(renderOptions.TempFolder, "icons"));
        this._purple = iconPurple;
        this._offline = renderOptions.Offline;
        this._fontSize = renderOptions.FontSize;
        this._outline = renderOptions.Outline;
        if (this._outline) {
            this._outlinePaint = outlinePaint.Clone();
            this._outlinePaint.StrokeWidth *= (float)(HighlightIcons.ICON_SIZE / (this._fontSize / 0.6));
        }
    }
    ...
    public static HighlightType GetHighlightType(Comment comment) {
        ...
        if (bodySpan.StartsWith(displayName)) {
            ...
            if (bodyWithoutName.Contains(
                    " consecutive streams this month and sparked a watch streak! ",
                    StringComparison.Ordinal
                ))
                return HighlightType.WatchStreak;

            if (bodyWithoutName.StartsWith(": Donated ")
                && bodyWithoutName[10..].Contains(" to support ", StringComparison.Ordinal))
                return HighlightType.CharityDonation;

            if (bodyWithoutName.StartsWith(" converted from a")) {
                // TODO: use bodyWithoutName when .NET 7
                var convertedToMatch = Regex.Match(
                    comment.message.body,
                    $@"(?<=^{comment.commenter.display_name} converted from a (?:Prime|Tier \d) sub to a )(?:Prime|Tier \d)"
                );
        ...
        if (char.IsDigit(bodySpan[0]) && bodySpan.EndsWith(" have joined! "))
            ...
            if (Regex.IsMatch(
                    comment.message.body,
                    $@"^\d+ raiders from {comment.commenter.display_name} have joined! "
                ))
                return HighlightType.Raid;

        const string ANONYMOUS_GIFT_ACCOUNT_ID = "274598607"; // Display name is 'AnAnonymousGifter'
        if (comment.commenter._id is ANONYMOUS_GIFT_ACCOUNT_ID
            && HighlightIcons.GiftAnonymousRegex.IsMatch(comment.message.body))
    ...
    public SKImage GetHighlightIcon(HighlightType highlightType, SKColor textColor) {
        return highlightType switch {
            HighlightType.SubscribedTier => this._subscribedTierIcon
                ??= this.GenerateSvgIcon(HighlightIcons.SUBSCRIBED_TIER_ICON_SVG, textColor),
            HighlightType.SubscribedPrime => this._subscribedPrimeIcon ??= this.GenerateSvgIcon(
                HighlightIcons.SUBSCRIBED_PRIME_ICON_SVG,
                this._purple
            ),
            HighlightType.GiftedSingle => this._giftSingleIcon
                ??= this.GenerateSvgIcon(HighlightIcons.GIFTED_SINGLE_ICON_SVG, textColor),
            HighlightType.GiftedMany => this._giftManyIcon ??= this.GenerateGiftedManyIcon(),
            HighlightType.GiftedAnonymous => this._giftAnonymousIcon
                ??= this.GenerateSvgIcon(HighlightIcons.GIFTED_ANONYMOUS_ICON_SVG, textColor),
            HighlightType.BitBadgeTierNotification => this._bitBadgeTierNotificationIcon ??= this.GenerateSvgIcon(
                HighlightIcons.BIT_BADGE_TIER_NOTIFICATION_ICON_SVG,
                textColor
            ),
            HighlightType.WatchStreak => this._watchStreakIcon
                ??= this.GenerateSvgIcon(HighlightIcons.WATCH_STREAK_ICON_SVG, textColor),
            HighlightType.CharityDonation => this._charityDonationIcon
                ??= this.GenerateSvgIcon(HighlightIcons.CHARITY_DONATION_ICON_SVG, textColor),
            _ => null
        };
    }
    ...
    private SKImage GenerateGiftedManyIcon() {
        ...
        var taskIconBytes = TwitchHelper.GetImage(
            this._cacheDir,
            HighlightIcons.GIFTED_MANY_ICON_URL,
            "gift-illus",
            3,
            "png",
    ...
    private SKImage GenerateSvgIcon(string iconSvgString, SKColor iconColor) {
        using var tempBitmap = new SKBitmap(HighlightIcons.ICON_SIZE, HighlightIcons.ICON_SIZE);
        using var tempCanvas = new SKCanvas(tempBitmap);

        var iconPaint = this.GetSvgIconPaint(iconColor);
        ...
        var newSize = (int)(this._fontSize / 0.6); // 20*20px @ 12pt font
    ...
    /// </summary>
    /// <returns>
    ///     2 clones of <paramref name="comment" /> whose <see cref="Message.body" /> and <see cref="Message.fragments" />
    ///     contain the split re-sub details and
    ...
    public static (Comment subMessage, Comment customMessage) SplitSubComment(Comment comment) {
        var subMessageMatch = HighlightIcons.SubMessageRegex.Match(comment.message.body);
        ...
        {
            customMessageComment.message.fragments[0].text = customMessage[..(customMessage.IndexOf(
                    comment.message.fragments[1].text,
                    StringComparison.Ordinal
                )
                - 1)];
            return (subMessageComment, customMessageComment);
    ...
    public static (Comment subMessage, Comment customMessage) SplitWatchStreakComment(Comment comment) {
        var watchStreakMatch = HighlightIcons.WatchStreakRegex.Match(comment.message.body);
    ...
    private void Dispose(bool isDisposing) {
        try {
            ...
            if (isDisposing) {
                this._subscribedTierIcon?.Dispose();
                this._subscribedPrimeIcon?.Dispose();
                this._giftSingleIcon?.Dispose();
                this._giftManyIcon?.Dispose();
                this._giftAnonymousIcon?.Dispose();
                this._bitBadgeTierNotificationIcon?.Dispose();
                this._outlinePaint?.Dispose();
                foreach (var (_, paint) in this._iconPaints)
                    paint.Dispose();
            }
        } finally {
            this.Disposed = true;
