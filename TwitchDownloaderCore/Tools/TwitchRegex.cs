...
public static class TwitchRegex {
    ...
    private static readonly Regex HighlightId = new(
        @"(?<=^|twitch\.tv\/\w+\/v(?:ideo)?\/)\d+(?=$|\?|\s)",
        RegexOptions.Compiled
    );

    private static readonly Regex ClipId = new(
        @"(?<=^|(?:clips\.)?twitch\.tv\/(?:\w+\/clip\/)?)[\w-]+?(?=$|\?|\s)",
        RegexOptions.Compiled
    );
    ...
    public static Match MatchVideoId(string text) {
        var videoIdMatch = TwitchRegex.VideoId.Match(text);
        if (videoIdMatch.Success)
            return videoIdMatch;

        var highlightIdMatch = TwitchRegex.HighlightId.Match(text);
    ...
    public static Match MatchClipId(string text) {
        var clipIdMatch = TwitchRegex.ClipId.Match(text);
