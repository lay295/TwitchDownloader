...
public static class FilenameService {
    ...
    private static void ReplaceCustomWithFormattable(
        StringBuilder sb,
        Regex regex,
        ...
        do {
            ...
            sb.Insert(
                ...
                ReplaceInvalidFilenameChars(formattable.ToString(formatString, formatProvider))
            );
        } while (true);
    }
    ...
    public static string ReplaceInvalidFilenameChars([AllowNull] string filename) {
        ...
        if (newName.AsSpan().IndexOfAny("\"*:<>?|/\\") != -1)
            newName = string.Create(
                filename.Length,
                filename,
                (span, str) => {
                    const int
                        FULL_WIDTH_OFFSET
                            = 0xFEE0; // https://en.wikipedia.org/wiki/Halfwidth_and_Fullwidth_Forms_(Unicode_block)
                    for (var i = 0; i < str.Length; i++) {
                        var ch = str[i];
                        span[i] = ch switch {
                            '\"' or '*' or ':' or '<' or '>' or '?' or '|' or '/' or '\\' => (char)(ch
                                + FULL_WIDTH_OFFSET),
                            _ => ch
                        };
                    }
        ...
        return newName.ReplaceAny(FilenameService.FilenameInvalidChars, '_');
    }
