...
public partial record M3U8(M3U8.Metadata FileMetadata, M3U8.Stream[] Streams) {
    public override string ToString() {
        ...
        if (this.FileMetadata?.ToString() is { Length: > 0 } metadataString)
    ...
    public partial record Metadata {
        ...
        public IReadOnlyList<KeyValuePair<string, string>> UnparsedValues => this._unparsedValues;

        public override string ToString() {
            var sb = new StringBuilder();
            var itemSeparator = Environment.NewLine;

            StringBuilderHelpers.AppendIfNotDefault(sb, Metadata.TARGET_VERSION_KEY, this.Version, itemSeparator);
            StringBuilderHelpers.AppendIfNotDefault(
                sb,
                Metadata.TARGET_DURATION_KEY,
                this.StreamTargetDuration,
                itemSeparator
            );
            if (this.Type != PlaylistType.Unknown) {
                sb.Append(Metadata.PLAYLIST_TYPE_KEY);
                sb.Append(this.Type.AsString());
                sb.Append(itemSeparator);
            }

            StringBuilderHelpers.AppendIfNotDefault(sb, Metadata.MEDIA_SEQUENCE_KEY, this.MediaSequence, itemSeparator);
            StringBuilderHelpers.AppendIfNotDefault(
                sb,
                Metadata.TWITCH_LIVE_SEQUENCE_KEY,
                this.TwitchLiveSequence,
                itemSeparator
            );
            StringBuilderHelpers.AppendIfNotDefault(
                sb,
                Metadata.TWITCH_ELAPSED_SECS_KEY,
                this.TwitchElapsedSeconds,
                itemSeparator
            );
            StringBuilderHelpers.AppendIfNotDefault(
                sb,
                Metadata.TWITCH_TOTAL_SECS_KEY,
                this.TwitchTotalSeconds,
                itemSeparator
            );

            foreach (var (key, value) in this._unparsedValues) {
                sb.Append(key);
                sb.Append(value);
                sb.Append(itemSeparator);
            }
    ...
    public partial record Stream(
        Stream.ExtMediaInfo MediaInfo,
        Stream.ExtStreamInfo StreamInfo,
        Stream.ExtPartInfo PartInfo,
        DateTimeOffset ProgramDateTime,
        Stream.ExtByteRange ByteRange,
        string Path
    ) {
        public Stream(ExtMediaInfo mediaInfo, ExtStreamInfo streamInfo, string path) : this(
            mediaInfo,
            streamInfo,
            null,
            default,
        ...
        public Stream(ExtPartInfo partInfo, DateTimeOffset programDateTime, ExtByteRange byteRange, string path) : this(
            null,
            null,
            partInfo,
            programDateTime,
        ...
        public readonly partial record struct ExtByteRange(uint Start, uint Length) {
            ...
            public override string ToString() => $"{ExtByteRange.BYTE_RANGE_KEY}{this.Start}@{this.Length}";

            public static implicit operator ExtByteRange((uint start, uint length) tuple)
                => new(tuple.start, tuple.length);
        }
        ...
        public partial record ExtMediaInfo {
            ...
            public override string ToString() {
                var sb = new StringBuilder(ExtMediaInfo.MEDIA_INFO_KEY);
                ...
                StringBuilderHelpers.AppendStringIfNotNullOrEmpty(sb, "GROUP-ID=", this.GroupId, keyValueSeparator);
                StringBuilderHelpers.AppendStringIfNotNullOrEmpty(sb, "NAME=", this.Name, keyValueSeparator);

                sb.Append("AUTOSELECT=");
                sb.Append(BooleanToWord(this.AutoSelect));
                sb.Append(keyValueSeparator);

                sb.Append("DEFAULT=");
                sb.Append(BooleanToWord(this.Default));
        ...
        public partial record ExtStreamInfo {
            ...
            public override string ToString() {
                var sb = new StringBuilder(ExtStreamInfo.STREAM_INFO_KEY);
                ReadOnlySpan<char> keyValueSeparator = stackalloc char[] { ',' };

                StringBuilderHelpers.AppendIfNotDefault(sb, "PROGRAM-ID=", this.ProgramId, keyValueSeparator);
                StringBuilderHelpers.AppendIfNotDefault(sb, "BANDWIDTH=", this.Bandwidth, keyValueSeparator);
                StringBuilderHelpers.AppendStringIfNotNullOrEmpty(sb, "CODECS=", this.Codecs, keyValueSeparator);
                StringBuilderHelpers.AppendIfNotDefault(sb, "RESOLUTION=", this.Resolution, keyValueSeparator);
                StringBuilderHelpers.AppendStringIfNotNullOrEmpty(sb, "VIDEO=", this.Video, keyValueSeparator);
                StringBuilderHelpers.AppendIfNotDefault(sb, "FRAME-RATE=", this.Framerate, default);
            ...
            public readonly partial record struct StreamResolution(uint Width, uint Height) {
                ...
                public static implicit operator StreamResolution((uint width, uint height) tuple)
        ...
        public partial record ExtPartInfo {
            ...
            public override string ToString() {
                var sb = new StringBuilder(ExtPartInfo.PART_INFO_KEY);
    ...
    private static class StringBuilderHelpers {
        ...
        public static void AppendIfNotDefault(
            StringBuilder sb,
            string keyName,
            Stream.ExtStreamInfo.StreamResolution value,
        ...
        public static void AppendStringIfNotNullOrEmpty(
            StringBuilder sb,
            string keyName,
            string value,
            ReadOnlySpan<char> end
