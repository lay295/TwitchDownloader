...
internal sealed record VideoDownloadThread {
    ...
    private TimeSpan VodAge => DateTimeOffset.UtcNow - this._vodAirDate;
    public Task ThreadTask { get; private set; }

    public void StartDownload() {
        if (this.ThreadTask is {
            ...
            throw new InvalidOperationException(
                $"Tried to start a thread that was already running or waiting to run ({this.ThreadTask.Status})."
            );
    ...
    private async Task DownloadVideoPartAsync(string videoPartName, CancellationTokenSource cancellationTokenSource) {
        ...
        while (true) {
            ...
            try {
                var partFile = Path.Combine(this._cacheFolder, DownloadTools.RemoveQueryString(videoPartName));
                long expectedLength;
                if (tryUnmute && videoPartName.Contains("-muted")) {
                    var unmutedPartName = videoPartName.Replace("-muted", "");
                    expectedLength = await DownloadTools.DownloadFileAsync(
                        this._client,
                        new(this._baseUrl, unmutedPartName),
                        partFile,
                        this._throttleKib,
                        this._logger,
                    ...
                    expectedLength = await DownloadTools.DownloadFileAsync(
                        this._client,
                        new(this._baseUrl, videoPartName),
                        partFile,
                        this._throttleKib,
                        this._logger,
                        cancellationTokenSource
                ...
                if (actualLength != expectedLength) {
                    ...
                    this._logger.LogVerbose(
                        $"{partFile} failed to verify: expected {expectedLength:N0}B, got {actualLength:N0}B."
                    );
                    if (++lengthFailureCount > MAX_RETRIES)
                        throw new(
                            $"Failed to download {partFile}: expected {expectedLength:N0}B, got {actualLength:N0}B."
                        );
                ...
                this._logger.LogVerbose(
                    $"Received {ex.StatusCode}: {ex.StatusCode} when trying to unmute {videoPartName}. Disabling {nameof(tryUnmute)}."
                );
                tryUnmute = false;
                ...
                this._logger.LogVerbose(
                    $"Received {(int)(ex.StatusCode ?? 0)}: {ex.StatusCode} for {videoPartName}. {MAX_RETRIES - (errorCount + 1)} retries left."
                );
