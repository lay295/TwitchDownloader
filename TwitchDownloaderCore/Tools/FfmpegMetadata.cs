...
public static class FfmpegMetadata {
    ...
    public static async Task SerializeAsync(
        ...
        IEnumerable<VideoMomentEdge> videoMomentEdges = null,
        CancellationToken cancellationToken = default
    ) {
        await using var fs = new FileStream(filePath, FileMode.Create, FileAccess.Write, FileShare.Read);
        await using var sw = new StreamWriter(fs) { NewLine = FfmpegMetadata.LINE_FEED };

        await SerializeGlobalMetadata(
            sw,
            streamerName,
            videoId,
            videoTitle,
    ...
    private static async Task SerializeChapters(
        StreamWriter sw,
        IEnumerable<VideoMomentEdge> videoMomentEdges,
    ...
    private static string SanitizeKeyValue(string str) {
        ...
        if (str.AsSpan().IndexOfAny(@$"=;#\{FfmpegMetadata.LINE_FEED}") == -1)
            return str;
        ...
        return new StringBuilder(str)
            ...
            .Replace("#", @"\#")
            .Replace(@"\", @"\\")
            .Replace(FfmpegMetadata.LINE_FEED, $@"\{FfmpegMetadata.LINE_FEED}")
            .ToString();
    }
}
