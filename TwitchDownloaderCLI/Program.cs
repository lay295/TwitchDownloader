...
internal static class Program {
    private static void Main(string[] args) {
        ...
        var parserResult
            = parser
                .ParseArguments<VideoDownloadArgs, ClipDownloadArgs, ChatDownloadArgs, ChatUpdateArgs, ChatRenderArgs,
                    FfmpegArgs, CacheArgs, TsMergeArgs>(preParsedArgs);
    ...
    private static void WriteHelpText(
        ...
        if (errors.FirstOrDefault()?.Tag == ErrorType.NoVerbSelectedError) {
            ...
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows)) {
                ...
                Console.WriteLine(
                    "This is a command line tool. Please open a terminal and run \"{0} help\" from there for more information.{1}Press any key to close...",
                    processFileName,
                    Environment.NewLine
                );
                Console.ReadKey();
                ...
                Console.WriteLine(
                    "Usage: {0} [VERB] [OPTIONS]{1}Try \'{2} help\' for more information.",
                    processFileName,
                    Environment.NewLine,
                    processFileName
                );
    ...
    private static void WriteApplicationBanner(ITwitchDownloaderArgs args) {
        ...
        var nameVersionString = HeadingInfo.Default.ToString();
            ...
            nameVersionString =
 System.Text.RegularExpressions.Regex.Replace(nameVersionString, @"(?<=\d)\+[0-9a-f]+", "");
#endif
