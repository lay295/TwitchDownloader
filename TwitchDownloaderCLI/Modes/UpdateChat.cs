...
namespace TwitchDownloaderCLI.Modes
{
    internal class UpdateChat
    {
        internal static void Update(ChatUpdateArgs inputOptions)
        {
            ...
            if (!inputOptions.EmbedMissing && double.IsNegative(inputOptions.CropBeginningTime) && double.IsNegative(inputOptions.CropEndingTime))
            {
                Console.WriteLine("[ERROR] - No update options were passed. Please pass --embed-missing, -b, or -e");
                Environment.Exit(1);
            }
