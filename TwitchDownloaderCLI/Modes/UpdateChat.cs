...
internal static class UpdateChat {
    ...
    private static ChatUpdateOptions GetUpdateOptions(
        ChatUpdateArgs inputOptions,
        ...
        if (Path.GetFullPath(inputOptions.InputFile!) == Path.GetFullPath(inputOptions.OutputFile!))
            logger.LogWarning(
                "Output file path is identical to input file. This is not recommended in case something goes wrong. All data will be permanently overwritten!"
            );
