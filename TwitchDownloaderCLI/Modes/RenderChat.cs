...
internal static class RenderChat {
    ...
    private static ChatRenderOptions GetRenderOptions(
        ChatRenderArgs inputOptions,
        FileCollisionHandler collisionHandler,
        ITaskLogger logger
    ) {
        ChatRenderOptions renderOptions = new() {
            ...
            MessageFontStyle = inputOptions.MessageFontStyle.ToLower() switch {
                ...
                _ => throw new NotSupportedException(
                    "Invalid message font style. Valid values are: normal, bold, and italic"
                )
            },
            UsernameFontStyle = inputOptions.UsernameFontStyle.ToLower() switch {
                ...
                "italic" or "italics" => SKFontStyle.Italic,
                _ => throw new NotSupportedException(
                    "Invalid username font style. Valid values are: normal, bold, and italic"
                )
            },
            UpdateRate = inputOptions.UpdateRate,
            Framerate = inputOptions.Framerate,
            GenerateMask = inputOptions.GenerateMask,
            InputArgs = inputOptions.Sharpening
                ? inputOptions.InputArgs + " -filter_complex \"smartblur=lr=1:ls=-1.0\""
            ...
            EmojiVendor = inputOptions.EmojiVendor.ToLower() switch {
                ...
                _ => throw new NotSupportedException(
                    "Invalid emoji vendor. Valid values are: 'twitter' / 'twemoji', 'google' / 'notocolor', and 'system' / 'none'"
                )
            },
        ...
        if (renderOptions.GenerateMask
            && renderOptions.BackgroundColor.Alpha == 255
            && !(renderOptions.AlternateMessageBackgrounds! && renderOptions.AlternateBackgroundColor.Alpha != 255))
            logger.LogWarning(
                "Generate mask option has been selected with an opaque background. You most likely want to set a transparent background with --background-color \"#00000000\""
            );

        if (renderOptions.ChatHeight % 2 != 0 || renderOptions.ChatWidth % 2 != 0) {
            logger.LogWarning(
                "Width and Height MUST be even, rounding up to the nearest even number to prevent errors"
            );
